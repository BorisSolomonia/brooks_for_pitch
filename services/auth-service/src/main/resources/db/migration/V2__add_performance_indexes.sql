...
CREATE INDEX IF NOT EXISTS idx_users_active ON users (status, created_at DESC)
  WHERE status = 'ACTIVE';
...
-- Index for token lookup (most common query) — partial on non-revoked tokens
...
-- Index for cleanup job (non-revoked tokens, sorted for expiry scan)
...
CREATE INDEX IF NOT EXISTS idx_user_devices_user ON user_devices (user_id, platform);
