...
CREATE INDEX IF NOT EXISTS idx_friendships_reverse ON friendships (friend_id, user_id, status);
COMMENT ON INDEX idx_friendships_reverse IS
  'Optimizes reverse friendship lookups';

-- Index for pending friend requests
CREATE INDEX IF NOT EXISTS idx_friendships_pending ON friendships (friend_id, status, requested_at DESC)
  WHERE status = 'PENDING';
COMMENT ON INDEX idx_friendships_pending IS
  'Optimizes friend request inbox queries';

-- Index for accepted friendships
CREATE INDEX IF NOT EXISTS idx_friendships_accepted ON friendships (user_id, accepted_at DESC)
...
CREATE INDEX IF NOT EXISTS idx_follows_active ON follows (follower_id, followee_id)
  WHERE status = 'ACTIVE';
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_relationship_prefs_lookup
...
CREATE INDEX IF NOT EXISTS idx_relationship_prefs_subject
