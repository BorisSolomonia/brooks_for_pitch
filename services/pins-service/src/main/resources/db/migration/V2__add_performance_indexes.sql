...
CREATE INDEX IF NOT EXISTS idx_pins_bucket_time_composite ON pins (bucket, expires_at, available_from);
...
CREATE INDEX IF NOT EXISTS idx_pins_active_geom ON pins USING GIST (geom);
COMMENT ON INDEX idx_pins_active_geom IS
  'Optimizes spatial queries for pins';
...
CREATE INDEX IF NOT EXISTS idx_pin_acl_pin_id ON pin_acl (pin_id);
COMMENT ON INDEX idx_pin_acl_pin_id IS
  'Optimizes ACL retrieval for access control evaluation';

-- Index for media lookups
CREATE INDEX IF NOT EXISTS idx_pin_media_pin_id ON pin_media (pin_id, sort_order);
...
CREATE INDEX IF NOT EXISTS idx_pin_notification_state_user ON pin_notification_state (user_id, pin_id);
...
CREATE INDEX IF NOT EXISTS idx_pins_expired ON pins (expires_at);
COMMENT ON INDEX idx_pins_expired IS
  'Optimizes expired pin cleanup job queries';

-- Index for owner queries (my pins)
CREATE INDEX IF NOT EXISTS idx_pins_owner_active ON pins (owner_id, expires_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_pins_future_self ON pins (owner_id, future_self)
  WHERE future_self = TRUE;
